...
V3의 목적은 V2에서 이미 정의된 케이스를 변경하지 않고, 이 문서만으로 구현 착수가 가능하도록 개발 규칙과 생성 규칙을 닫는 것이다.
...
v1 구현 기준에서 각 케이스는 아래 실행파일에 속한다.
...
```text
- 이 파일은 수기 편집 대상이 아니라 manifest에서 생성되는 결과물이다.
- 아래 예시는 win_core에 속한 케이스만 포함해야 한다.
- C++/POSIX 전용 케이스는 이 registry에 들어가면 안 된다.
```
...
```text
1. 선언 순서와 g_cases[] 순서는 manifest 순서를 따른다.
2. runtime 파일은 대응 binary의 케이스만 포함한다.
3. 사람이 runtime 파일을 직접 수정하지 않는다.
4. runtime 파일 재생성 후 validate_manifest.py로 manifest = registry 일치를 검증한다.
```
...
```text
- expected/*.expected.json은 수기 편집하지 않는다.
- 항상 manifest에서 생성한다.
- 비교기는 generated_from과 program 값을 이용해 대상 binary와 정답 파일의 짝이 맞는지 먼저 확인한다.
```
...
```json
{
    ...
    "binaries": [
        ...
        },
        {
            ...
            "platform": "posix",
            "kind": "runtime_bridge",
            "runtime_file": "src/dfbench_runtime_posix.c",
            "expected_file": "expected/dfbench_posix_runtime.expected.json"
        },
        {
            "name": "dfbench_cpp",
            "platform": "windows",
            "kind": "cpp",
            "runtime_file": "src/dfbench_runtime_cpp.c",
            "expected_file": "expected/dfbench_cpp.expected.json"
        },
        {
            "name": "dfbench_cpp_exceptions",
            "platform": "windows",
            "kind": "cpp_exceptions",
            "runtime_file": "src/dfbench_runtime_cpp_exceptions.c",
            "expected_file": "expected/dfbench_cpp_exceptions.expected.json"
        }
    ],
    "cases": [
        {
        ...
        {
            "id": "DFB090",
            "name": "thread_shared_memory",
            "binary": "dfbench_posix_runtime",
            "source_file": "src/cases_runtime_bridge.c",
...
```text
1. validate_manifest.py가 schema와 중복 ID, binary 소유권을 검증한다.
2. generate_registry_from_manifest.py가 각 runtime 파일을 생성한다.
3. generate_expected_from_manifest.py가 expected/*.expected.json을 생성한다.
4. smoke_test.py와 consistency 검증이 manifest = registry = expected json 일치를 확인한다.
```

### 19.3 수기 편집 대상과 생성 대상
...
6. binary.runtime_file과 generated runtime 파일 경로가 일치해야 한다.
7. binary.expected_file과 generated expected JSON 경로가 일치해야 한다.
```
...
$executables = @(
    ...
    "build/win-debug/dfbench_posix_runtime.exe",
...
for exe in "$ROOT"/build/*/dfbench_win_core "$ROOT"/build/*/dfbench_posix_runtime "$ROOT"/build/*/dfbench_cpp "$ROOT"/build/*/dfbench_cpp_exceptions; do
    if [[ -x "$exe" ]]; then
        echo "[*] $exe --list"
        "$exe" --list
        echo "[*] $exe --run-all"
...
def main():
    ...
    candidates.extend(root.glob("build/**/dfbench_posix_runtime"))
    candidates.extend(root.glob("build/**/dfbench_posix_runtime.exe"))
...
src/dfbench_sources_sinks.c
manifests/cases_manifest.json
tools/generate_registry_from_manifest.py
...
```text
manifest validation이 통과한다.
win_core runtime 파일과 expected JSON이 manifest에서 생성된다.
```
...
```text
DFB080/DFB081은 dfbench_cpp에만 포함한다.
DFB111은 dfbench_cpp_exceptions에만 포함한다.
```
...
```text
expected/*.expected.json이 모두 generator로 생성된다.
수기 작성 expected JSON이 남아 있지 않다.
manifest = runtime registry = expected json 일치 검증이 통과한다.
```
...
if built, run dfbench_posix_runtime --list
if built, run dfbench_posix_runtime --run-all
run dfbench_cpp --list
run dfbench_cpp --run-all
run dfbench_cpp_exceptions --list
run dfbench_cpp_exceptions --run-all
...
3. source/sink 함수 작성
4. cases manifest 작성
5. manifest validator 작성
6. runtime registry generator 작성
7. expected JSON generator 작성
8. main CLI 작성
9. C core case 파일 작성
10. C++ case 파일 작성
11. importlib shared library 작성
12. CMakeLists.txt/CMakePresets 작성
13. scripts 작성
14. smoke_test.py 작성
15. manifest 기반 생성 + 로컬 빌드 검증
...
- runtime registry는 manifest에서 생성한다
- 실행파일별 expected JSON은 각 registry와 정확히 일치
- expected JSON은 manifest에서 생성한다
...
2. dfbench_cpp와 dfbench_cpp_exceptions 바이너리가 생성된다.
3. POSIX 환경에서는 dfbench_posix_runtime 바이너리가 생성된다.
4. 각 실행파일의 --list가 해당 DFB 케이스만 출력한다.
5. 각 실행파일의 --run-all이 crash 없이 종료된다.
6. 실행파일별 expected JSON이 존재하며, 대응 registry와 정확히 같은 케이스 집합을 포함한다.
7. 모든 case 함수가 심볼 테이블에 남아 있다.
8. Windows 기준 O0/O2/LTO 빌드가 가능하다.
9. dfbench_importlib shared library가 생성되고 필요한 실행파일에서 사용된다.
10. smoke_test.py가 통과한다.
11. manifest = registry = expected json 일치 검증이 통과한다.
12. runtime/expected 산출물에 수기 편집분이 남아 있지 않다.
```
